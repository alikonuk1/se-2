...
import "./utils/token/ERC721/IERC721.sol";
import "./utils/token/ERC721/utils/ERC721Holder.sol";
import "./utils/token/ERC1155/IERC1155.sol";
import "./utils/token/ERC1155/utils/ERC1155Holder.sol";
import "./utils/token/ERC1155/utils/ERC1155Receiver.sol";
 ...
 * @dev This contract is for lending and borrowing non-fungible tokens (NFTs)
 * @dev Please reach out to ali@pxswap.xyz if you find any issues
 */

contract PxLend is ILendData, Ownable, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    function borrow(
        ...
        bundleCall(
            handleBorrow, createBorrowCallData(nftStandard, nftAddress, tokenID, _lendingID, borrowDuration, borrowAmount)
        );
    }
    ...
    function handleStopLend(ILendData.CallData memory cd) internal {
        ...
        for (uint256 i = cL; i < cR;) {
            bytes32 lendingIdentifier =
                keccak256(abi.encodePacked(cd.nftAddress[cL], cd.tokenID[i], cd.lendingID[i]));
        ...
        safeTransfer(
            ...
            address(this),
            msg.sender,
            sliceArr(cd.tokenID, cL, cR, 0),
            sliceArr(lentAmounts, cL, cR, cL)
        );
    }
    ...
    function handleBorrow(ILendData.CallData memory cd) internal {
        ...
        for (uint256 i = cL; i < cd.right;) {
            ...
            bytes32 lendingIdentifier =
                keccak256(abi.encodePacked(cd.nftAddress[cL], id, cd.lendingID[i]));
    ...
    function handleStopBorrow(ILendData.CallData memory cd) private {
        ...
        for (uint256 i = cL; i < cd.right;) {
            ...
            bytes32 borrowingIdentifier =
                keccak256(abi.encodePacked(nft, id, bId));
            ...
            manageWillAutoRenew(
                lending, borrowing, nft, cd.nftStandard[cL], id, lId
            );
        ...
        if (lending.willAutoRenew == false) {
            if (amount > amountB) {
                ...
                IERC1155(nftAddress).safeTransferFrom(
                    address(this), lender, tokenID, uint256(amountB), ""
                );
            }
    ...
    function bundleCall(function(ILendData.CallData memory) handler, ILendData.CallData memory cd) internal {
        ...
        while (cr != nft.length) {
            if (
                (nft[cl] == nft[cr])
                    && (cd.nftStandard[cr] == ILendData.NFTStandard.E1155)
            ) {
                ++cr;
            } else {
                handler(cd);
